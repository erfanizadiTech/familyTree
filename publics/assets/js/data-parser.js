...
export function transformDataToTree(rawPeopleData) {
    ...
    rawPeopleData.forEach(p => {
        ...
        peopleMap.set(p.PersonId, {
            ...p,
            id: p.PersonId, // D3 often uses 'id', so map PersonId to id
            children: [], // D3 hierarchy children
    ...
    const allChildNodes = new Set(); // Stores all nodes that are children in the hierarchy
    peopleMap.forEach(p => p.children.forEach(child => allChildNodes.add(child.id))); // Children of persons
    marriageNodesMap.forEach(m => m.children.forEach(child => allChildNodes.add(child.id))); // Children of marriages

    let potentialRoots = [];
    peopleMap.forEach(person => {
        if (!allChildNodes.has(person.id)) {
            potentialRoots.push(person);
        }
    });

    // Merge individual roots with their spouse's marriage node if they are also roots
    let finalRoots = [];
    const processedRootIds = new Set(); // To prevent adding the same person/marriage twice

    potentialRoots.forEach(person => {
        if (!processedRootIds.has(person.id)) {
            let foundMarriageRoot = false;
            // Check if this person is part of a marriage node that is also a root
            if (person.Marriage && person.Marriage.length > 0) {
                ...
                if (spouseMarriage) {
                    ...
                    // Check if the spouse is also a potential root and the marriage node exists
                    const spouseIsRoot = potentialRoots.some(r => r.id === spouseId);

                    if (marriageNode && spouseIsRoot) {
                        finalRoots.push(marriageNode); // Use the marriage node as a top-level root
            ...
            if (!foundMarriageRoot) {
                finalRoots.push(person); // Add individual as a root if not part of a top-level marriage
                processedRootIds.add(person.id);
            }
        }
    });
